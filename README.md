...
## Spring Boot and GraalVM native-build sample application with Testcontainers test

Source code that could be found here in this repo shows, how to solve problem mentioned above.
It basically creates shared Named Volume that is used as holder for input and output files that are used and generated by running containers.

There is `WorkingDirectories` abstraction that is used to support actions with files/directories when we run application using standard JVM.
Or when we run application inside Docker as container using Docker in Docker approach (DinD).

There are two test classes:
- `DockerInceptionMountApplicationTests` which run standard application tests as JVM process.
- `NativeTests` which first build GraalVM native-image Docker Image and then run tests in it.
    - If you are curious why not to run Spring Boot `-P nativeTest`, please check discussion in the [issue tracker](https://github.com/spring-projects/spring-boot/issues/49316).

What sapmle application does:
1. Sample application generates input file, pass it to the CLI.
2. Then run two Testcontainers containers - each generates own output file with content from input file concatenated with random text.
3. When both containers finish it's work, final output file is generated by combining both output files from containers.
